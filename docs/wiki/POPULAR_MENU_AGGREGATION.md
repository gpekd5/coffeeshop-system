...
현재 구현은 Redis 캐시가 아니라 내부 주문 DB를 직접 집계한다.
정확한 정책 검증을 먼저 끝낸 뒤, 트래픽이 늘어나는 상황에서 Redis 캐시나 별도 집계 저장소를 비교할 수 있도록 기준선을 만든다.
...
| 개선 방향 | 설명 |
| --- | --- |
| Redis 캐시 | 짧은 TTL로 인기 메뉴 결과 캐싱. 메뉴/주문 변경 시 무효화 기준 필요 |
