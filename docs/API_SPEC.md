...
- 1차 구현에서는 전송 성공, 실패, Timeout을 `external_order_event_logs`에 기록한다.
- 외부 API 실패나 Timeout은 이미 완료된 주문을 Rollback하지 않는다.
...
Mock API는 다음 테스트용 Query Parameter를 받을 수 있다.

| 이름 | 설명 | 기본값 |
|---|---|---:|
| `delayMillis` | 응답 전 대기 시간(ms) | `0` |
| `status` | 반환할 HTTP 상태 코드 | `200` |
