...
| Docker | Docker Compose 지원 버전 | 로컬 MySQL, Redis, Kafka 실행 |
| MySQL | 8.4 | 운영/로컬 데이터베이스 |
| Redis | 7.4 | 토큰 상태 저장 |
| Kafka | Docker Kafka 3.7 | Outbox 이벤트 발행 및 Consumer 처리 검증 |
...
로컬 `compose.yaml`에는 MySQL, Redis, Kafka가 포함되어 있습니다.
Kafka Publisher/Consumer는 기본 비활성화이며, 환경변수로 명시 활성화할 때만 동작합니다.

---

## 로컬 실행

MySQL, Redis, Kafka를 실행합니다.
...
| `APP_ORDER_EVENT_DELIVERY_MODE` | `outbox` | 주문 이벤트 전송 방식. `sync`, `outbox` |
...
| 모드 | 설명 |
| --- | --- |
| `sync` | 주문 Commit 이후 외부 Mock API를 동기 호출해 주문 응답시간에 외부 지연을 포함 |
| `outbox` | 주문 트랜잭션에서 Outbox를 저장하고 Kafka Publisher/Consumer가 외부 API를 비동기로 호출 |

테스트 결과는 k6 `--summary-export` JSON을 CSV로 누적한 뒤 SVG 그래프로 변환할 수 있습니다.
